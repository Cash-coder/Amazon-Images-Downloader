...
input_file = 'phones_missing.xlsx' #source file to make the requests
output_file = '2_phones_matches_2.xlsx' #used to save the results
no_results = 'phones_noresults.xlsx' 
...
def make_query_url(item,attribute):
    ...
    #url = 'https://www.amazon.es/s?k=lenovo+legion+5&i=electronics&__mk_es_ES=%C3%85M%C3%85%C5%BD%C3%95%C3%91&ref=nb_sb_noss_2'
...
def make_request(url):
    ...
    with d:
        ...
        sleep(2)
        # cookies_agreement = d.find_element_by_xpath('//input[@id="sp-cc-accept"]')
        # if cookies_agreement:
        #     cookies_agreement.click()
        # sleep(3)
...
# def take_screenshot(d,url):
#     d.save_screenshot("screenshot"+url+".png")
...
def get_prod_matches(prods,item_p,attribute_p,query):
    ...
    for p in prods:
        try:
            #title
            try:
                title = p.find_element_by_xpath('.//span[contains(@class,"a-size-medium a-color-base a-text-normal")]').text
...
for e in target_list:
    try:
        ...
        try:
            ...
            print("exception in prods = make_request(url)")
            print(e)
            continue
        ...
        print('exception inside main loop for e in target_list:')
        print(e)
        continue